...
def load_model(file_path):
    ...
    try:
        ...
        return None, f"Error loading model file ({file_path}): {e}"

# File paths to models
file_paths = {
    "Parkinson's Disease": "parkinsons.pkl",
...
# Common function to display errors and stop execution
def display_error_and_stop(error):
    st.error(error)
    st.stop()
...
if selected_model == "Parkinson's Disease":
    ...
    model_path = file_paths["Parkinson's Disease"]
    model, error = load_model(model_path)

    if error:
        display_error_and_stop(error)
    
    try:
        feature_names = model.feature_names_in_
    except AttributeError:
        display_error_and_stop("Feature names are not available in the model.")
...
# Kidney Disease Prediction
    ...
    model_path = file_paths["Kidney Disease"]
    model, error = load_model(model_path)
